...
namespace SSD_Components
{
    ...
    void TSU_Priority_OutOfOrder::Schedule()
    {
        ...
        if (_NVMController->check_idle_controller_exist()) {                                    //只有存在空闲控制器才会分配请求
            ...
            for (flash_channel_ID_type channelID = 0; channelID < channel_count; channelID++)
            {
                ...
                //{
               if (!_NVMController->check_idle_controller_exist()) {
                    break;
                    }
               for (unsigned int i = 0; i < chip_no_per_channel; i++)//把通道上的所有chip都遍历一遍
               {
                    ...
                    if (!service_read_transaction(chip))//无论chip状态如何都会执行service_read_transaction，因为就算chip正处于busy状态，也会suspend这个chip上的事务然后发送命令                                                               
                    {
    ...
    bool TSU_Priority_OutOfOrder::service_read_transaction(NVM::FlashMemory::Flash_Chip* chip)      //注意sourceQueue是一个chip级别的事务队列
    {
        ...
        else if (ftl->GC_and_WL_Unit->GC_is_in_urgent_mode(chip))                           //进入这个说明没有映射读事件，且gc处于紧急模式
        {
            ...
            {
                sourceQueue1 = get_next_read_service_queue(chip);                           //进入这个队列说明几个队列都没有待处理的事务
        ...
        {                                                                                   //没有映射表映射事务，gc也不处于紧急模式，处于抢占模式，此时优先检查用户队列                             
            ...
            {
                return false;                                                               //若读队列没有合适的内容，但是写队列有合适的内容，返回false回到process_chip_request函数服务写请求
            }
            else if (GCReadTRQueue[chip->ChannelID][chip->ChipID].size() > 0)               //此时读队列和写队列都没有合适的内容
            {
                if (Simulator->Time() == 3380269) {
                    std::cout << "4" << std::endl;
                }
        ...
        //实际上，只有ChipStatus的状态为IDLE才能继续执行下去，因为下面这个switch函数没有除了状态为IDLE时都没有break语句，导致必定会执行return false
        ...
        case ChipStatus::WRITING:
            ...
            if (_NVMController->Expected_finish_time(chip) - Simulator->Time() < writeReasonableSuspensionTimeForRead)//说明写操作即将完成，不需要挂起
            {
                return false;
            }
        ...
        if (Simulator->Time() == 5416806601) {
            int static count = 1;
            std::cout << "before count is " << count << " size is " << MappingReadTRQueue[chip->ChannelID][chip->ChipID].size() << std::endl;
            count++;
        }

        issue_command_to_chip(sourceQueue1, sourceQueue2, Transaction_Type::READ, suspensionRequired, chip->ChannelID, chip->ChipID);//由于没有break语句，当执行issue_command_to_chip函数时，suspensionRequired的值一定为false；

        if (Simulator->Time() == 5416806601) {
            int static vcount = 1;
            std::cout << "after count is " << vcount << " size is " << MappingReadTRQueue[chip->ChannelID][chip->ChipID].size() << std::endl;
            vcount++;
        }
        return true;
    }
    ...
    bool TSU_Priority_OutOfOrder::service_write_transaction(NVM::FlashMemory::Flash_Chip* chip)
    {
        ...
        issue_command_to_chip(sourceQueue1, sourceQueue2, Transaction_Type::WRITE, suspensionRequired, chip->ChannelID, chip->ChipID);
    ...
    bool TSU_Priority_OutOfOrder::service_erase_transaction(NVM::FlashMemory::Flash_Chip* chip)
    {
        ...
        issue_command_to_chip(source_queue, NULL, Transaction_Type::ERASE, false, chip->ChannelID, chip->ChipID);
